...
20250418: 你可以使用该项目收集到优质的tool call数据,也可以在任何地方使用本项目准备好的与cursor基本一致的mcp服务(除却一个检索code的mcp,个人使用时发现效果不大),另外,本项目额外添加了web搜索以及安全连接sql和clickhouse数据库,经过测试,发现诸多模型皆有危险操作可能,因此与数据库相关操作均限制为只读.
